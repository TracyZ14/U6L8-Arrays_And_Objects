public class Fibonacci
{
    ...
    {
        int valueIndex = -1;
        for(int i = 0; (i < sequence.length) && (valueIndex == -1); i++)
        {
    ...
    {
        int[] extended = new int[sequence.length + howManyMore];
        for(int i = 0; i < sequence.length; i++)
        {
            extended[i] = sequence[i];
        }
        for(int i = sequence.length; i < extended.length; i++)
        {
            extended[i] = extended[i - 2] + extended[i - 1];
        }
        sequence = extended;
    }
}